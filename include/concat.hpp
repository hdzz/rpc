...
namespace fnk
{
    ...
    struct is_concatable_instance
        : std::conditional_t
            <fnk::rfoldable<C>::is_rfoldable_instance::value &&
             fnk::monoid<typename fnk::type_support::container_traits<C>::value_type>
                ::is_monoid_instance::value,
             std::true_type, std::false_type> {};

    template <class C,
        typename = std::enable_if_t<fnk::rfoldable<C>::is_rfoldable_instance::value>,
        typename = std::enable_if_t<is_concatable_instance<C>::value>>
    inline constexpr decltype(auto) concat (C const& c)
    {
        ...
        return fnk::rfoldable<C>::template foldr
            ([](auto const& l, auto const& r)
                { return fnk::append<decltype(l),decltype(r)>(l, r); },
            fnk::monoid<A>::unity(),
            c); 
    }
} // namespace fnk
